...
function showInterface() {
    (interfaceAnimationIn = new TimelineMax({
        ...
        interfaceAnimationIn.to(camera.position, 5, { x: randomIntFromInterval(-4, 4)*10, y: randomIntFromInterval(-4, 4)*10,z: randomIntFromInterval(-8, -1)*10, ease: "expo.inOut" }, 2),
        interfaceAnimationIn.to(camera.rotation, 5, { x: randomIntFromInterval(-3, 3)*10 * toRAD, y: randomIntFromInterval(-3, 3)*10 * toRAD, z: randomIntFromInterval(-3, 3)*10 * toRAD, ease: "expo.inOut" }, 2),
...
function resetPosition() {
    resetMovement(),
        ...
        gsap.to(camera.position, 5, { x: randomIntFromInterval(-4, 4)*10, y: randomIntFromInterval(-4, 4)*10,z: randomIntFromInterval(-8, -1)*10, ease: "expo.out" }),
        gsap.to(camera.rotation, 5, { x: randomIntFromInterval(-3, 3)*10 * toRAD, y: randomIntFromInterval(-3, 3)*10 * toRAD, z: randomIntFromInterval(0, 3)*10 * toRAD, ease: "expo.out" });
}
var lightObject,
    lightClose,
    lightFar,
...
function render() {
    if ((scene.updateMatrixWorld(), isWarpComplete)) {
        ...
        var t = 0.1 * rateRotationX,
            e = 0.1 * rateRotationY,
            o = 0.1 * rateRotationZ;
        ($("#pitch .rate").innerHTML = t.toFixed(1) + " \xB0/s"), ($("#yaw .rate").innerHTML = e.toFixed(1) + " \xB0/s"), ($("#roll .rate").innerHTML = o.toFixed(1) + " \xB0/s");
        ...
        ($("#x-range .distance").innerHTML = a.toFixed(1) + " m"), ($("#y-range .distance").innerHTML = r.toFixed(1) + " m"), ($("#z-range .distance").innerHTML = i.toFixed(1) + " m");
        var n = Math.sqrt(a * a + r * r + i * i);
        if ((($("#range .rate").innerHTML = n.toFixed(1) + " m"), (rangeRateCounter += 1), 20 <= rangeRateCounter)) {
            rangeRateCounter = 0;
            var s = Date.now();
            (rateCurrent = 0.75 * (1e3 * ((n - prevRange) / (s - prevRangeTime)))),
...
function checkCollision() {
    if (!0 !== isGameOver) {
        ...
        if (
            ...
            0 < t.length && (($("#fail-message").innerHTML = "You made contact with the International Space Station."), hideInterface("fail")),
            500 < e && (($("#fail-message").innerHTML = "You are too far away from the International Space Station."), hideInterface("fail")),
            0.2 > e && -150 < camera.position.z)
        )
            if (o <= toleranceRotation && a <= toleranceRotation && r <= toleranceRotation && i <= toleranceRate && n <= toleranceRate && s <= toleranceRate) hideInterface("success");
            else {
                var l = "";
